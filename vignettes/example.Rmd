...
if (FALSE) {
hydrusR::create.H1D.project(project.name = project_name, 
...
hydrusR::create.soil.profile(project.path = project_path,
                             profile.depth = profile_depth,
                             dz = deltaz,
                             obs.nodes = NULL)

##Write root distribution
hydrusR::write.obs.nodes(project.path = project_path,
                         obs.nodes = obs_nodes_all)

hydrusR::write.ini.cond(project.path = project_path,
                        wt.depth = initial_wtable)

hydrusR::write.root.dist(project.path = project_path,
                         rdepth = rooting_depth,
                         rBeta = 0.962)

hydrusR::write.hydraulic.para(project.path = project_path,
                              para = soil_para)

hydrusR::write.bottom.bc(constant.bc = TRUE,
                         bc.type = bot_bc_type,
                         bc.value = const_botFlux,
                         project.path = project_path)

hydrusR::write.atmosph.in(project.path = project_path,
                          maxAL = 2000, 
                          deltaT = time_step,
...
hydrusR::run.H1D.simulation(project.path = project_path,
                            profile.depth = profile_depth,
                            beginT = 0,
                            endT = endTime,
                            deltaT = time_step,
